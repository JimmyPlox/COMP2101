...
echo "Performing command processing:
 "
...
while [ $# -gt 0 ]; do
  case "$1" in
    -h | --help )
      echo "Use [-h|--help] to access this command"
      exit
      ;;
    -d )
      debug="yes"
  ...
  echo "
  There are $# things left to process on the command line."
...
#         Tell the user if verbose mode is on
